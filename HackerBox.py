...
def HackerBoxupdate():
    ...
    def updateCheck():
        ...
        sleep(1)
        review_url = "https://pastebin.com/kU4sqcN5"
        resp = requests.get(review_url)
        soup = BeautifulSoup(resp.text, 'html.parser')
    ...
    def oldVersionCheckdownload():
        ...
        import os
        import zipfile
        review_url = "https://pastebin.com/u8ttUEs5"
        versionReturn()
        resp = requests.get(review_url)
        soup = BeautifulSoup(resp.text, 'html.parser')
        old_ver_check = soup.find("textarea").text
...
if __name__ == "__main__":
    try:
        ...
        license3 = f"""{Fore.RED}Hacker Box Copyright (C) 2022  Muhammad Sami Furqan
...
under certain conditions;"""
    ...
    while True:
        ...
        sql_completer = WordCompleter(['scanfix', 'hacking.scripts', 'root.showip', 'wifi/nby', 'wlan/show.pass', 'lan.ping',
                                       ...
                                       'pass.bruteforce', 'admin.panelfinder', 'xss/webchecker', 'sitetester', '?', 'help', 'dos.start', 'ftp.bruteforce','port.discovery','web.bruteforce'], ignore_case=True)
